...
| 13 | Platform services | Every shipped project | Staged (`staging/platform-services`) | Entitlement, achievements, leaderboards, cloud save, and store sign-in as thin adapters over the vendor SDKs behind one contract; no shipped product avoids it and nothing in the library covered it before the gap matrix |
| 14 | Quality tiers | Every shipped project | Staged (`staging/quality-tiers`) | Refresh rate, render scale, foveation, and per-device presets; store certification requires a stable frame rate on every headset |
| 15 | Content packs | Most projects | Candidate, source gate passed | Data-driven loading of levels, packs, and user content behind one contract (Addressables or equivalent); custom levels and user scenes are how XR products grow after release |
| 16 | Spatial placement | Creation tools, many games | Candidate, source gate passed | Ray placement with ghost preview, sticky aim hand, stick-as-distance, snapping, commit and cancel, and locomotion suppression while aiming; from the creation-tool shape of the gap matrix |
