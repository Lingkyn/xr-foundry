...
| Renderer-neutral UI contract with sibling Unity renderers | `unity-ui-toolkit-world-space`, `unity-ui-toolkit-world-space-create`, `unity-nested-prefabs` | Semantic view state and intents remain renderer-neutral; UGUI prefabs and UI Toolkit visual trees are peer adapters |
...
| Optional XR world-space presentation and device acceptance | `unity-xri`, `unity-xri-ui-toolkit-support`, `unity-xri-world-space-ui-sample`, `unity-xri-examples`, `pico-unity-project-validation`, `pico-unity-controller-ray-canvas`, `pico-unity-build-run`, `pico-unity-input-mapping`, `pico-unity-openxr-release-notes` | XR belongs in renderer-explicit adapters using their supported tracked-device UI paths; configuration and automated routing are prerequisites, while each renderer/device tuple needs separate installation, interaction, readability, anchoring, and comfort evidence |
