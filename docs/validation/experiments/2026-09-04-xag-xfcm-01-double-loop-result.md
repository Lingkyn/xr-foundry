...
Unity-tested source commit: `2605c1276c9a60065db8fa49ddd37c18c29e814d`

Post-test XFCM authority hardening commit:
`be9cd0dfbc30ee03ff2f3a7dd933309f68b0c8cd`
...
- Unity result acceptance gate: `d203960`;
- bounded evidence documentation: `2605c12`; and
- strict XFCM authority validation: `be9cd0d`.
...
| 8 | Final scope review reproduced duplicate-key last-wins parsing, invalid numeric-prerelease SemVer acceptance and an arbitrary C# file/assembly pair producing `bindings_implemented: true` | B | Reject duplicate JSON keys at any depth, apply SemVer 2.0 prerelease rules to v0.2, and bind each source to its nearest unique regular same-name `.asmdef`; unsupported `.asmref` fails closed | 11 focused tests, a 1,050-value SemVer corpus, current compose check and the full 154-test contract passed; current lock stayed byte-identical |
| 9 | Enabling the strict loader exposed unhandled duplicate-JSON exceptions in existing-lock and Device Lab profile/plan CLI paths | B | Catch the new authority error at each public CLI boundary and return structured failure instead of traceback | Adversarial CLI cases failed closed and the complete repository contract passed again |
...
The Unity-tested source commit was materialized into a path that did not exist
before the command. Every Unity result also used a new filename. The epoch was
recorded immediately before its Unity command; the verifier rejected any result
predating that boundary and required the independently source-counted total.
...
Repository gates at the Unity-tested commit and after the post-test B-layer
hardening:
...
The post-test hardening additionally passed 11 focused cases, an independent
1,050-value SemVer corpus with zero Python/schema mismatches, and the complete
154-test repository contract. The v0.1 schemas and current v0.2 lock remained
byte-identical. From the Unity-tested commit through that hardening, no consumer,
package C#, asmdef, Unity manifest, materializer or Unity-result-verifier path
changed; the prior Unity execution therefore remains bound to identical Unity
inputs while the repository authority gate became stricter.
...
- XFCM v0.2 can represent and content-bind the three implemented consumer-owned
  adapter sources, verify their owning Unity assembly definitions and honestly
  retain `runtime_ready: false`.
- XFCM authority JSON now rejects duplicate keys at any depth, and v0.2 SemVer
  plus adapter assembly ownership fail closed under the exercised mutations.
...
- An explicitly supplied in-memory `composition_payload` can still differ from the
  on-disk bytes whose digest is recorded. The public CLI loads one disk source and
  cannot reach that mismatch; fixing the internal API correctly requires separating
  candidate validation from durable lock generation rather than changing raw-byte
  hash semantics. `.asmref` ownership is likewise unsupported and fails closed.
...
startup/rollback/reverse-stop semantics, complete source/revision binding, a split
candidate-validation/durable-generation API, exact offline dependency provisioning,
Settings load resynchronization, and later named device evidence. None is silently
...
failure recovery passes; the stricter XFCM authority generator and all repository
contracts pass without changing the Unity inputs or current lock; every observed
blocking defect was routed and regenerated; and the Supervisor intentionally
invoked no prohibited account-setting, wallet, chain or remote-repository
operation. Unity's automatic pre-existing license-client behavior is the disclosed
exception to a broader "no external side effect" reading.
