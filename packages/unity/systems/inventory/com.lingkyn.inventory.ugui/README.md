...
It contains no XR Interaction Toolkit or
...
## Renderer-neutral presentation namespace

`InventoryUiState`, `InventorySlotViewModel`, `InventoryViewModel`,
`InventorySlotIntent`, `IInventoryView`, and `InventoryPresenter` belong to
...
Assembly definitions that directly use those contracts must reference
`Lingkyn.Inventory.Presentation`. Renderer components remain in
`Lingkyn.Inventory.UGUI`.

For Git evaluation, explicitly list Core, Presentation, and UGUI at the same full
