...
2. Freeze consumer-owned plain DTO snapshots (`[Serializable]` fields only; no `UnityEngine.Object`, dictionaries, delegates, or scene graph state).
...
- Initial create commits use staged move semantics and remain distinct from replacement commits.
- `RecoverableReplace` and `BestEffortWrite` are separate, conservative claims.
