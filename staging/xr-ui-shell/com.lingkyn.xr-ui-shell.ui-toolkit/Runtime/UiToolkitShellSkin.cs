...
{
    // The UI Toolkit adapter's one injectable skin seam: a ScriptableObject carrying a concrete
    // value for every shared design-language token this scaffold uses, with a default asset that
    // carries the canonical values from docs/standards/design-language/ui-design-language-standard.json
    // so an un-themed install already matches the shared look. This type shares nothing with the
    // UGUI sibling's UguiShellSkin beyond the Core token and slot names both map: no colour,
    // radius, or hit-size value lives anywhere else in this package. A bound document's own view
    // writes every visual value as a USS custom property or class through this seam and this seam
    // alone.
    ...
    [CreateAssetMenu(menuName = "Lingkyn/XR UI Shell/UI Toolkit Skin", fileName = "XrUiShellUiToolkitSkin")]
    public sealed class UiToolkitShellSkin : ScriptableObject
    {
        ...
        /// this skin holds no colour for; a caller only asks a colour-valued token because the
        /// slot it is resolving for is itself colour-valued.</summary>
        ...
        /// <summary>The USS custom property name a real view writes this slot's colour or size
        /// under (for example <c>--shell-panel-background</c>). Pure text formatting; this seam
        /// never touches a VisualElement itself.</summary>
        public static string UssVariableName(ShellSlot slot) => "--shell-" + ShellSlotNames.ToName(slot).Replace('.', '-');
    }
}
